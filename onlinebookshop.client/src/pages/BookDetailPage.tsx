// BookDetailPage.tsx —
// =============================================

import { useState, useEffect } from "react";
import { useParams, useNavigate, Link } from "react-router-dom";
import axios from "axios";
import Navbar from "../components/common/Navbar";
import ReviewSection from "../components/books/ReviewSection";
import BookService from "../services/book.service";
import CartService from "../services/cart.service";
import ReviewService from "../services/review.service";
import AuthService from "../services/auth.service";
import type { BookResponseDto, CategoryResponseDto } from "../types/book.types";
import type { ReviewResponseDto } from "../types/review.types";
import WishlistButton from "../components/wishlist/WishlistButton";

const PLACEHOLDER_IMG = "https://placehold.co/400x550/f1f5f9/94a3b8?text=No+Cover";

type DetailTab = "summary" | "description" | "reviews";

export default function BookDetailPage() {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();

    const [book, setBook] = useState<BookResponseDto | null>(null);
    const [reviews, setReviews] = useState<ReviewResponseDto[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState("");
    const [categories, setCategories] = useState<CategoryResponseDto[]>([]);
    const [addingToCart, setAddingToCart] = useState(false);
    const [cartMessage, setCartMessage] = useState("");
    const [cartError, setCartError] = useState("");
    const [cartQuantity, setCartQuantity] = useState(0);
    const [activeTab, setActiveTab] = useState<DetailTab>("summary");

    useEffect(() => {
        const fetchData = async () => {
            if (!id) return;
            setLoading(true);
            setError("");
            try {
                const [bookData, catsData] = await Promise.all([
                    BookService.getBookById(parseInt(id)),
                    BookService.getCategories(),
                ]);
                setBook(bookData);
                setCategories(catsData);
                try {
                    const reviewsData = await ReviewService.getByBook(parseInt(id));
                    setReviews(reviewsData);
                } catch {
                    setReviews([]);
                }
            } catch (err: unknown) {
                if (axios.isAxiosError(err)) {
                    setError(err.response?.data?.message || "Book not found.");
                } else {
                    setError("Something went wrong.");
                }
            } finally {
                setLoading(false);
            }
        };
        fetchData();
    }, [id]);

    useEffect(() => {
        if (!AuthService.isAuthenticated() || !id) return;
        const loadCartQty = async () => {
            try {
                const cart = await CartService.getMyCart();
                const item = cart.items.find((i) => i.bookId === parseInt(id));
                setCartQuantity(item?.quantity ?? 0);
            } catch {
                setCartQuantity(0);
            }
        };
        loadCartQty();
    }, [id]);

    const handleAddToCart = async () => {
        if (!AuthService.isAuthenticated()) { navigate("/auth"); return; }
        if (!book) return;
        setAddingToCart(true);
        setCartError("");
        setCartMessage("");
        try {
            await CartService.addItem({ bookId: book.bookId, quantity: 1 });
            setCartMessage("Book added to cart!");
            setCartQuantity((prev) => prev + 1);
            window.dispatchEvent(new Event("cartUpdated"));
            setTimeout(() => setCartMessage(""), 2500);
        } catch (err: unknown) {
            if (axios.isAxiosError(err)) {
                setCartError(err.response?.data?.message || "Failed to add to cart.");
            } else {
                setCartError("Something went wrong.");
            }
            setTimeout(() => setCartError(""), 3000);
        } finally {
            setAddingToCart(false);
        }
    };

    const handleReviewAdded = (review: ReviewResponseDto) => setReviews((prev) => [review, ...prev]);
    const handleReviewDeleted = (reviewId: number) => setReviews((prev) => prev.filter((r) => r.reviewId !== reviewId));

    const avgRating = reviews.length > 0
        ? reviews.reduce((sum, r) => sum + r.rating, 0) / reviews.length : 0;

    const isOutOfStock = book?.stock === 0;
    const isMaxedOut = book ? cartQuantity >= book.stock : false;
    const categoryName = categories.find((c) => c.categoryId === book?.categoryId)?.name ?? "—";

    const hasDiscount = !!book?.discountPrice && book.discountPrice < book.price;
    const displayPrice = hasDiscount ? book!.discountPrice! : book?.price ?? 0;
    const discountPercentage = hasDiscount
        ? Math.round(((book!.price - book!.discountPrice!) / book!.price) * 100) : 0;

    // ── Specification rows (single source of truth — shown once, in Summary tab) ──
    const summaryRows = book ? [
        { label: "Author", value: book.author, icon: "bi-person" },
        { label: "Category", value: categoryName, icon: "bi-tag" },
        { label: "Publisher", value: book.publisher, icon: "bi-building" },
        { label: "Edition", value: book.edition, icon: "bi-journal-bookmark" },
        { label: "Language", value: book.language, icon: "bi-translate" },
        { label: "Pages", value: book.pageCount ? `${book.pageCount} pages` : null, icon: "bi-file-text" },
        { label: "ISBN", value: book.isbn, icon: "bi-upc-scan" },
    ].filter((row) => row.value) : [];

    // ── Design tokens ──
    const INK = "#1c1917";
    const PAPER = "#faf7f2";
    const MAROON = "#9f1239";
    const NAVY = "#0f172a";
    const SERIF = "Georgia, 'Noto Serif', 'Times New Roman', serif";

    return (
        <>
            <Navbar />
            <div style={{ background: PAPER, minHeight: "100vh" }} className="py-4">
                <div className="container" style={{ maxWidth: "1140px" }}>

                    {/* Breadcrumb */}
                    <nav className="mb-4" aria-label="breadcrumb">
                        <ol className="breadcrumb mb-0" style={{ fontSize: "13px" }}>
                            <li className="breadcrumb-item">
                                <Link to="/" className="text-decoration-none" style={{ color: "#78716c" }}>Home</Link>
                            </li>
                            {book && !loading && (
                                <li className="breadcrumb-item">
                                    <span style={{ color: "#78716c" }}>{categoryName}</span>
                                </li>
                            )}
                            <li className="breadcrumb-item active text-truncate" style={{ maxWidth: "300px", color: INK }}>
                                {loading ? "Loading..." : book?.title ?? "Book Detail"}
                            </li>
                        </ol>
                    </nav>

                    {/* Loading */}
                    {loading && (
                        <div className="text-center py-5">
                            <div className="spinner-border" style={{ width: "2.5rem", height: "2.5rem", color: MAROON }} />
                            <p className="mt-3 small" style={{ color: "#78716c" }}>Loading book details...</p>
                        </div>
                    )}

                    {/* Error */}
                    {!loading && error && (
                        <div
                            className="d-flex align-items-center gap-2 rounded-3 border-0 shadow-sm px-4 py-3"
                            style={{ background: "#fef2f2", color: "#9f1239" }}
                        >
                            <i className="bi bi-exclamation-triangle-fill"></i>
                            {error}
                            <Link to="/" className="btn btn-link btn-sm ms-auto p-0" style={{ color: MAROON }}>Go back to Home</Link>
                        </div>
                    )}

                    {!loading && book && (
                        <>
                            {/* ══════════════ Top Section: one unified compact card ══════════════ */}
                            <div
                                className="mb-4 bg-white"
                                style={{
                                    borderRadius: "16px",
                                    border: "1px solid #e7e5e4",
                                    boxShadow: "0 10px 30px -14px rgba(28,25,23,0.20)",
                                    overflow: "hidden",
                                }}
                            >
                                <div className="d-flex flex-column flex-lg-row">

                                    {/* ── Cover ── */}
                                    <div className="p-4" style={{ flex: "0 0 auto", width: "220px", maxWidth: "100%", margin: "0 auto" }}>
                                        <div
                                            className="overflow-hidden position-relative bg-white"
                                            style={{
                                                borderRadius: "4px 10px 10px 4px",
                                                boxShadow: "0 14px 30px -10px rgba(28,25,23,0.28), 0 2px 6px rgba(28,25,23,0.08)",
                                                borderLeft: `5px solid ${NAVY}`,
                                            }}
                                        >
                                            {hasDiscount && (
                                                <div style={{
                                                    position: "absolute", top: 0, left: 0,
                                                    width: 0, height: 0, borderStyle: "solid",
                                                    borderWidth: "58px 58px 0 0",
                                                    borderColor: `${MAROON} transparent transparent transparent`,
                                                    zIndex: 2,
                                                }}>
                                                    <div style={{
                                                        position: "absolute", top: "-52px", left: "1px",
                                                        color: "#fff", fontSize: "10px", fontWeight: 500,
                                                        lineHeight: 1.15, transform: "rotate(-45deg)",
                                                        textAlign: "center", width: "42px",
                                                    }}>
                                                        <span style={{ fontSize: "13px", fontWeight: 700 }}>{discountPercentage}%</span>
                                                        <br />
                                                        <span style={{ fontSize: "8px", letterSpacing: "0.05em" }}>OFF</span>
                                                    </div>
                                                </div>
                                            )}
                                            <img
                                                src={book.imageUrl || PLACEHOLDER_IMG}
                                                alt={book.title}
                                                className="w-100"
                                                style={{ aspectRatio: "3/4.2", objectFit: "cover", display: "block" }}
                                                onError={(e) => (e.currentTarget.src = PLACEHOLDER_IMG)}
                                            />
                                        </div>

                                        {/* Status badges below cover */}
                                        <div className="d-flex gap-2 mt-3 flex-wrap">
                                            {book.isFeatured && (
                                                <span
                                                    className="d-inline-flex align-items-center gap-1 px-3 py-2"
                                                    style={{ background: "#fffbeb", color: "#92400e", fontSize: "11px", fontWeight: 600, borderRadius: "999px", border: "1px solid #fde68a" }}
                                                >
                                                    <i className="bi bi-star-fill"></i>Featured
                                                </span>
                                            )}
                                            {/*{!book.isActive && (*/}
                                            {/*    <span className="badge rounded-pill px-3 py-2 bg-secondary" style={{ fontSize: "11px" }}>*/}
                                            {/*        Inactive*/}
                                            {/*    </span>*/}
                                            {/*)}*/}
                                        </div>
                                    </div>

                                    {/* ── Title, Meta, Description snippet ── */}
                                    <div
                                        className="p-4"
                                        style={{ flex: "1 1 auto", minWidth: 0, borderLeft: "1px dashed #e7e5e4", borderTop: "none" }}
                                    >

                                        <h1
                                            className="fw-bold mb-2"
                                            style={{ fontFamily: SERIF, fontSize: "26px", lineHeight: 1.25, color: NAVY, letterSpacing: "-0.01em" }}
                                        >
                                            {book.title}
                                        </h1>
                                        <p className="mb-3" style={{ fontSize: "15px" }}>
                                            <span style={{ color: "#78716c" }}>by </span>
                                            <span className="fw-semibold" style={{ color: MAROON }}>{book.author}</span>
                                        </p>

                                        {/* Rating row */}
                                        <div className="d-flex align-items-center gap-2 mb-2">
                                            {reviews.length > 0 ? (
                                                <>
                                                    <div className="d-flex gap-1">
                                                        {[1, 2, 3, 4, 5].map((star) => (
                                                            <i key={star}
                                                                className={`bi ${star <= Math.round(avgRating) ? "bi-star-fill" : "bi-star"}`}
                                                                style={{ fontSize: "15px", color: star <= Math.round(avgRating) ? "#d97706" : "#d6d3d1" }}
                                                            />
                                                        ))}
                                                    </div>
                                                    <span className="fw-semibold small" style={{ color: NAVY }}>{avgRating.toFixed(1)}</span>
                                                    <span className="small" style={{ color: "#78716c" }}>({reviews.length} review{reviews.length !== 1 ? "s" : ""})</span>
                                                </>
                                            ) : (
                                                <span className="small" style={{ color: "#78716c" }}>
                                                    <i className="bi bi-star me-1"></i>No reviews yet
                                                </span>
                                            )}
                                        </div>

                                        {/* Wishlist action — separate line below */}
                                        <div className="d-flex align-items-center gap-1 mb-3">
                                            <WishlistButton bookId={book.bookId} />
                                            <span className="small" style={{ color: "#78716c" }}>Add to Wishlist</span>
                                        </div>

                                        {/* Quick meta chips */}
                                        <div className="d-flex flex-wrap gap-2 mb-4">
                                            {[
                                                { label: "Category", icon: "bi-tag", text: categoryName },
                                                book.publisher ? { label: "Publisher", icon: "bi-building", text: book.publisher } : null,
                                            ].filter(Boolean).map((chip) => (
                                                <span
                                                    key={chip!.label}
                                                    className="d-inline-flex align-items-center gap-1 px-3 py-1"
                                                    style={{
                                                        background: "#f5f5f4",
                                                        color: "#44403c",
                                                        fontSize: "12px",
                                                        fontWeight: 500,
                                                        width: "fit-content",
                                                        borderRadius: "6px",
                                                        border: "1px solid #e7e5e4",
                                                    }}
                                                >
                                                    <i className={`bi ${chip!.icon}`} style={{ color: MAROON }}></i>
                                                    <span className="fw-semibold" style={{ color: NAVY }}>{chip!.label}:</span>
                                                    {chip!.text}
                                                </span>
                                            ))}
                                        </div>

                                        {/* Short description preview */}
                                        {book.description && (
                                            <p
                                                className="mb-0"
                                                style={{
                                                    color: "#57534e",
                                                    fontSize: "14.5px",
                                                    lineHeight: 1.75,
                                                    display: "-webkit-box",
                                                    WebkitLineClamp: 3,
                                                    WebkitBoxOrient: "vertical" as const,
                                                    overflow: "hidden",
                                                }}
                                            >
                                                {book.description}
                                            </p>
                                        )}
                                    </div>

                                    {/* ── Buy Box ── */}
                                    <div
                                        className="p-4"
                                        style={{
                                            flex: "0 0 auto",
                                            width: "300px",
                                            maxWidth: "100%",
                                            background: "#faf7f2",
                                            borderLeft: "1px dashed #e7e5e4",
                                        }}
                                    >
                                        {/* Price */}
                                        <div className="mb-3 pb-3" style={{ borderBottom: "1px dashed #e7e5e4" }}>
                                            {hasDiscount ? (
                                                <>
                                                    <div className="d-flex align-items-baseline gap-2">
                                                        <span className="fw-bold" style={{ fontSize: "28px", color: NAVY }}>
                                                            ৳{displayPrice.toFixed(0)}
                                                        </span>
                                                        <span className="text-decoration-line-through" style={{ fontSize: "15px", color: "#a8a29e" }}>
                                                            ৳{book.price.toFixed(0)}
                                                        </span>
                                                    </div>
                                                    <span
                                                        className="d-inline-block mt-1"
                                                        style={{ background: "#fef2f2", color: MAROON, fontSize: "11px", fontWeight: 600, padding: "3px 10px", borderRadius: "999px" }}
                                                    >
                                                        Save {discountPercentage}%
                                                    </span>
                                                </>
                                            ) : (
                                                <span className="fw-bold" style={{ fontSize: "28px", color: NAVY }}>
                                                    ৳{book.price.toFixed(0)}
                                                </span>
                                            )}
                                        </div>

                                        {/* Stock status */}
                                        <div
                                            className="d-flex align-items-center gap-2 mb-3 px-3 py-2"
                                            style={{
                                                background: isOutOfStock ? "#fef2f2" : "#f0fdf4",
                                                fontSize: "13px",
                                                borderRadius: "8px",
                                            }}
                                        >
                                            <i className={`bi ${isOutOfStock ? "bi-x-circle-fill text-danger" : "bi-check-circle-fill text-success"}`}></i>
                                            <span className={isOutOfStock ? "text-danger fw-semibold" : "text-success fw-semibold"}>
                                                {isOutOfStock ? "Out of Stock" : isMaxedOut ? "Max quantity in cart" : `${book.stock} in stock`}
                                            </span>
                                        </div>

                                        {cartMessage && (
                                            <div className="d-flex align-items-center gap-2 py-2 px-3 small mb-3" style={{ background: "#f0fdf4", color: "#15803d", borderRadius: "8px" }}>
                                                <i className="bi bi-check-circle-fill"></i>{cartMessage}
                                            </div>
                                        )}
                                        {cartError && (
                                            <div className="py-2 px-3 small mb-3" style={{ background: "#fef2f2", color: "#9f1239", borderRadius: "8px" }}>{cartError}</div>
                                        )}

                                        {cartQuantity > 0 && (
                                            <div className="d-flex align-items-center justify-content-between mb-3 px-3 py-2" style={{ background: "#f5f5f4", fontSize: "13px", borderRadius: "8px" }}>
                                                <span style={{ color: "#57534e" }}>
                                                    <i className="bi bi-cart-check me-1 text-success"></i>
                                                    {cartQuantity} in cart
                                                </span>
                                                <Link to="/cart" className="text-decoration-none fw-semibold" style={{ fontSize: "12px", color: MAROON }}>
                                                    View →
                                                </Link>
                                            </div>
                                        )}

                                        <button
                                            className="btn w-100 fw-bold py-2 mb-2 border-0"
                                            style={{
                                                background: isOutOfStock || isMaxedOut ? "#a8a29e" : NAVY,
                                                color: "#fff",
                                                borderRadius: "8px",
                                                fontSize: "14px",
                                            }}
                                            onClick={handleAddToCart}
                                            disabled={isOutOfStock || addingToCart || isMaxedOut}
                                        >
                                            {addingToCart ? <><span className="spinner-border spinner-border-sm me-2" />Adding...</>
                                                : isOutOfStock ? "Out of Stock"
                                                    : isMaxedOut ? `In Cart (${cartQuantity})`
                                                        : <><i className="bi bi-cart-plus me-2"></i>Add to Cart</>}
                                        </button>

                                        {cartQuantity > 0 && (
                                            <Link
                                                to="/cart"
                                                className="btn w-100 fw-semibold"
                                                style={{ background: "#fef2f2", color: MAROON, borderRadius: "8px", fontSize: "13px" }}
                                            >
                                                <i className="bi bi-cart3 me-2"></i>Go to Cart
                                            </Link>
                                        )}

                                        {/* Trust signals */}
                                        <div className="mt-3 pt-3" style={{ borderTop: "1px solid #e7e5e4" }}>
                                            {[
                                                { icon: "bi-truck", text: "Fast home delivery" },
                                                { icon: "bi-arrow-repeat", text: "Easy returns" },
                                                { icon: "bi-shield-check", text: "Secure payment" },
                                            ].map((t) => (
                                                <div key={t.text} className="d-flex align-items-center gap-2 mb-1">
                                                    <i className={`bi ${t.icon}`} style={{ fontSize: "13px", color: "#a8a29e" }}></i>
                                                    <span style={{ fontSize: "12px", color: "#78716c" }}>{t.text}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>

                                </div>
                            </div>

                            {/* ══════════════ Bottom Section: Tabs ══════════════ */}
                            <div className="row">
                                <div className="col-lg-9">
                                    <div
                                        className="bg-white overflow-hidden"
                                        style={{ borderRadius: "12px", border: "1px solid #e7e5e4", boxShadow: "0 4px 16px -8px rgba(28,25,23,0.08)" }}
                                    >
                                        {/* Tab headers */}
                                        <div style={{ borderBottom: "1px solid #f5f5f4" }}>
                                            <div className="d-flex" style={{ overflowX: "auto" }}>
                                                {[
                                                    { key: "summary" as DetailTab, label: "Specification", icon: "bi-card-list" },
                                                    { key: "description" as DetailTab, label: "Description", icon: "bi-text-left" },
                                                    { key: "reviews" as DetailTab, label: `Reviews (${reviews.length})`, icon: "bi-star" },
                                                ].map((tab) => (
                                                    <button
                                                        key={tab.key}
                                                        className="btn border-0 rounded-0 px-4 py-3 fw-semibold"
                                                        onClick={() => setActiveTab(tab.key)}
                                                        style={{
                                                            fontSize: "13.5px",
                                                            whiteSpace: "nowrap",
                                                            color: activeTab === tab.key ? MAROON : "#78716c",
                                                            borderBottom: activeTab === tab.key ? `2.5px solid ${MAROON}` : "2.5px solid transparent",
                                                            background: "transparent",
                                                            transition: "color 0.15s",
                                                        }}
                                                    >
                                                        <i className={`bi ${tab.icon} me-2`}></i>
                                                        {tab.label}
                                                    </button>
                                                ))}
                                            </div>
                                        </div>

                                        <div className="p-4">

                                            {/* ── Specification Tab ── */}
                                            {activeTab === "summary" && (
                                                summaryRows.length === 0 ? (
                                                    <p className="mb-0 small" style={{ color: "#78716c" }}>No specification details available.</p>
                                                ) : (
                                                    <div className="table-responsive">
                                                        <table className="table table-borderless mb-0" style={{ fontSize: "13.5px" }}>
                                                            <tbody>
                                                                {summaryRows.map((row, i) => (
                                                                    <tr key={row.label} style={{ background: i % 2 === 0 ? "#faf7f2" : "transparent" }}>
                                                                        <td
                                                                            className="fw-semibold"
                                                                            style={{ width: "180px", color: "#78716c", padding: "12px 16px", borderRadius: "8px 0 0 8px" }}
                                                                        >
                                                                            <i className={`bi ${row.icon} me-2`} style={{ color: "#a8a29e" }}></i>
                                                                            {row.label}
                                                                        </td>
                                                                        <td style={{ color: NAVY, fontWeight: 500, padding: "12px 16px", borderRadius: "0 8px 8px 0" }}>
                                                                            {row.value}
                                                                        </td>
                                                                    </tr>
                                                                ))}
                                                            </tbody>
                                                        </table>
                                                    </div>
                                                )
                                            )}

                                            {/* ── Description Tab ── */}
                                            {activeTab === "description" && (
                                                <div>
                                                    {book.description ? (
                                                        <p className="mb-0" style={{ color: "#44403c", lineHeight: 1.85, fontSize: "14.5px" }}>
                                                            {book.description}
                                                        </p>
                                                    ) : (
                                                        <p className="mb-0 small" style={{ color: "#78716c" }}>No description available for this book.</p>
                                                    )}
                                                </div>
                                            )}

                                            {/* ── Reviews Tab ── */}
                                            {activeTab === "reviews" && (
                                                <ReviewSection
                                                    bookId={book.bookId}
                                                    reviews={reviews}
                                                    onReviewAdded={handleReviewAdded}
                                                    onReviewDeleted={handleReviewDeleted}
                                                />
                                            )}

                                        </div>
                                    </div>
                                </div>
                            </div>
                        </>
                    )}

                </div>
            </div>
        </>
    );
}